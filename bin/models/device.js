...
function loadDeviceModel(sequelize) {
    let modFiles = [];
    (0, fs_1.readdirSync)((0, path_1.join)(__dirname, '../mods')).forEach(mod => {
        if ((0, fs_1.existsSync)((0, path_1.join)(__dirname, `../mods/${mod}/device`)))
            modFiles.push(require(`../mods/${mod}/device`));
    });
    let model = defaultModel;
    modFiles.forEach(load => model = Object.assign(Object.assign({}, model), load(sequelize_1.DataTypes)));
    return sequelize.define('Device', model, {
        timestamps: true,
        createdAt: 'created',
        updatedAt: 'updated'
    });
}
exports.default = loadDeviceModel;
